...
const props = defineProps({
  ...
  elCardClass: propTypes.bool.def(true),
  marginBottom: propTypes.string.def("10px"),
})
...
const newSchema = computed(() => {
  ...
  // 直接通过判断schema的长度，来决定是否展示更多
  propsComputed.showExpand = propsComputed.showExpand && schema.length > 3 || false
  if (propsComputed.showExpand && !unref(visible)) {
    const index = 3
...
const getSearchParam = async () => {
  const elFormExpose = await getElFormExpose()
  if (!elFormExpose) return // 确保 elFormExpose 存在

  try {
    const isValid = await elFormExpose.validate()
    if (isValid) {
      const model = await filterModel()
      return model
    }
...
<template>
  <Form
    ...
    :class="[getProps.inline ? 'search-inline-box' : '', getProps.elCardClass ? 'card-container' : '']"
