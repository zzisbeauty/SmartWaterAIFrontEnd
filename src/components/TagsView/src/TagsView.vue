...
const initTags = () => {
  ...
  for (const tag of unref(affixTagArr)) {
    // Must have tag name
    if (tag.name) {
      tagsViewStore.addVisitedView(cloneDeep(tag as any))
    }
  }
}
...
const getMenuRouters = () => {
  // parent 根据currentRoute获得
  const parentRoute = ref()
  // 根据 currentRoute 中的 matched 属性获取所有匹配的路由记录
  const matchedRoutes = unref(currentRoute).matched
  // 获取当前路由在匹配记录中的索引
  const currentRouteIndex = matchedRoutes.findIndex(
    (route) => route.path === unref(currentRoute).fullPath
  )
  // 确保找到了匹配的路由
  if (currentRouteIndex !== -1) {
    // 获取父路由的索引
    const parentRouteIndex = currentRouteIndex - 1
    // 检查是否有父路由
    if (parentRouteIndex >= 0) {
      // 获取父路由信息
      parentRoute.value = matchedRoutes[parentRouteIndex]

      // 现在 parentRoute 包含了父路由的信息，可以根据需要进行处理
      console.log('父路由信息:', parentRoute)
    }
  }
  const updatedMenuItems =
    parentRoute.value && parentRoute.value.children
      ? parentRoute.value.children.map((menuItem) => {
          return {
            path: pathResolve(parentRoute.value.path, menuItem.path),
            name: menuItem.name || '',
            meta: { ...menuItem.meta }
          }
        })
      : []
  permissionStore.setMenuTabRouters(updatedMenuItems)
}
