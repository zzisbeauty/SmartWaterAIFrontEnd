...
export default defineComponent({
  ...
  setup() {
    ...
    const tabClick = (item: AppRouteRecordRaw) => {
      ...
      if (item.children) {
        ...
        if (unref(showMenu)) {
          ...
          // 处理子菜单项的选择，这里选择第一个子菜单项
          const firstSubmenuItem = item.children[0]
          if (firstSubmenuItem) {
            handleSubmenu(firstSubmenuItem, newPath)
          }
        }
      } else {
        push(item.path)
    ...
    const handleSubmenu = (submenuItem: any, parentPath: string) => {
      const fullPath = pathResolve(parentPath, submenuItem.path)
      if (submenuItem.children) {
        const firstSubmenuItem = submenuItem.children[0]
        if (firstSubmenuItem) {
          handleSubmenu(firstSubmenuItem, fullPath)
        }
      } else {
        push(fullPath)
      }
    ...
    const isActive = (currentPath: string) => {
      ...
      return false
      // return tabPathMap[currentPath]?.length > 0
    }
                ...
                return (
                  ...
                  >
                    ...
                    {!unref(showTitle) ? undefined : (
                      <div class="break-words mt-0px px-2px">{t(item.meta?.title || '')}</div>
                    )}
                  </div>
                )
              })
...
:v-deep(.@{elNamespace}-scrollbar__wrap) {
  padding: 5px 7px;
}
