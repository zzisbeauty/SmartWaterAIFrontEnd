...
const importUrl = (url: string) => {
  return new URL(`/src/assets/imgs/${url}.png`, import.meta.url).href
}
const state = reactive({
  onlineUsers: '1,234',
  onlineUsersChangeRate: '+1.09%',
  totalUsageTime: '4,234h',
  totalUsageTimeChangeRate: '+1.34%',
  serviceProviderCount: '34',
  serviceProviderCountChangeRate: '-0.22%',
  usageCount: '15,692',
  usageCountChangeRate: '+1.25%'
})
const stats = computed(() => [
  {
    title: '当前在线用户数',
    value: state.onlineUsers,
    changeRate: state.onlineUsersChangeRate,
    compareText: '较昨日',
    icon: new URL(`@/assets/imgs/用户.png`, import.meta.url).href
  },
  {
    title: '今日总使用时长',
    value: state.totalUsageTime,
    changeRate: state.totalUsageTimeChangeRate,
    compareText: '较昨日',
    icon: new URL(`@/assets/imgs/时长.png`, import.meta.url).href
  },
  {
    title: '服务商数量',
    value: state.serviceProviderCount,
    changeRate: state.serviceProviderCountChangeRate,
    compareText: '较上月',
    icon: new URL(`@/assets/imgs/服务商.png`, import.meta.url).href
  },
  {
    title: '今日使用人次',
    value: state.usageCount,
    changeRate: state.usageCountChangeRate,
    compareText: '较昨日',
    icon: new URL(`@/assets/imgs/人次.png`, import.meta.url).href
  }
])
// 使用记录
...
<template>
  <div class="data-board w-full h-full">
    <!-- 顶部统计信息 -->
    <div class="w-full flex justify-between gap-3 p-b-3 h-150px">
      <div
        v-for="stat in stats"
        :key="stat.title"
        class="w-1/4 text-center pt-10px pb-20px pl-30px pr-30px flex items-center bg-white"
      >
        <div class="flex-1 text-left">
          <span class="lh-40px">{{ stat.title }}</span>
          <div class="stat-value flex items-center mb-10px">
            <span class="text-3xl font-bold">{{ stat.value }}</span>
          </div>
          <div class="stat-value flex items-center">
            <img
              v-if="stat.changeRate.startsWith('+')"
              src="@/assets/imgs/上升.png"
              :alt="`${stat.title}上升图标`"
            />
            <img
              v-else-if="stat.changeRate.startsWith('-')"
              src="@/assets/imgs/下降.png"
              :alt="`${stat.title}下降图标`"
            />
            <span
              :class="[
                stat.changeRate.startsWith('+') ? 'text-green-600' : 'text-red-600',
                'text-12px'
              ]"
            >
              {{ stat.changeRate.replace(/[+-]/, '') }}
            </span>
            <span class="text-12px ml-10px text-gray-500">{{ stat.compareText }}</span>
          </div>
        </div>
        <img :src="stat.icon" class="w-12 h-12 ml-auto" />
      </div>
    </div>
    <div class="w-full flex justify-between gap-3 p-b-3" style="height: calc(100% - 164px)">
      <!-- 左侧 -->
