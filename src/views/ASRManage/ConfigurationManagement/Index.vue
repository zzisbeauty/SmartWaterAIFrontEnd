...
// 配置热词表
const dialogVisibleHotWords = ref(false)
const dialogHotWordsTitle = ref('热词表配置')
const hotWords = ref()

// 配置热词表
const handleHotWords = async (row: any) => {
  dialogVisibleHotWords.value = true
  dialogHotWordsTitle.value = `热词表配置`
  await nextTick()
  hotWords.value.feedBackData(row)
}
const dialogHotWordsConfirm = debounce(async () => {
  if (await hotWords.value.submit()) {
    dialogVisibleHotWords.value = false
    getList()
  }
}, 500)
const dialogVisibleKeywords = ref(false)
const dialogKeywordsTitle = ref('关键词配置')
const keywords = ref()
// 配置关键词
const handleKeywords = async (row: any) => {
  dialogVisibleKeywords.value = true
  dialogKeywordsTitle.value = `关键词配置`
  await nextTick()
  keywords.value.feedBackData(row)
}
const dialogKeywordsConfirm = async () => {
  if (await keywords.value.submit()) {
    dialogVisibleKeywords.value = false
    getList()
  }
}
...
<template>
  ...
  <!-- 详情 -->
  <Dialog maxWidth="30%" maxHeight="45%" :title="dialogDetailTitle" v-model="dialogVisibleDetail">
    <Detail ref="detail" />
  </Dialog>
  <!-- 配置热词表 -->
  <Dialog
    maxWidth="45%"
    maxHeight="75%"
    :title="dialogHotWordsTitle"
    v-model="dialogVisibleHotWords"
  >
    <HotWords ref="hotWords" />
    <template #footer>
      <BaseButton @click="dialogVisibleHotWords = false">取消</BaseButton>
      <BaseButton type="primary" @click="dialogHotWordsConfirm">确定</BaseButton>
    </template>
  </Dialog>
  <!-- 配置关键词 -->
  <Dialog
    maxWidth="45%"
    maxHeight="65%"
    :title="dialogKeywordsTitle"
    v-model="dialogVisibleKeywords"
  >
    <KeyWords ref="keywords" />
    <template #footer>
      <BaseButton @click="dialogVisibleKeywords = false">取消</BaseButton>
      <BaseButton type="primary" @click="dialogKeywordsConfirm">确定</BaseButton>
    </template>
  </Dialog>
</template>
