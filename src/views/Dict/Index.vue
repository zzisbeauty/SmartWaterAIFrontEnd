...
import Config from './components/DetailTable/Index.vue'
...
const setConfig = (row) => {
  setDialog(true)
  nextTick(() => {
    configRef.value.setDictId(row.id)
  })
}
...
<template>
  <div class="w-100% h-100% table-page-container">
    ...
    <Dialog
      :maxWidth="dialogTitle == '参数配置' ? '500px' : '70%'"
      :maxHeight="dialogTitle == '参数配置' ? '300px' : '70%'"
      :title="dialogTitle"
      v-model="dialogVisible"
    >
      <Edit v-if="dialogTitle == '新增' || dialogTitle == '编辑'" ref="editRef" />
      <Config v-if="dialogTitle == '字典配置'" ref="configRef" />
      <template #footer>
        <BaseButton @click="dialogVisible = false">取消</BaseButton>
        <BaseButton type="primary" @click="dialogConfirm" v-if="dialogTitle != '字典配置'">
          确定
        </BaseButton>
      </template>
    </Dialog>
